...
module.exports = async message => {
    ...
    if (!(message.author.id in recently_posted))
        recently_posted[message.author.id] = []

    // push message to queue
    recently_posted[message.author.id].push(message)

    // remove oldest item from queue if over max length
    if (recently_posted[message.author.id].length > MESSAGES_TO_CACHE)
        recently_posted[message.author.id].shift()

    // check if all 3 messages are equal and are NOT stickers/attachments without message content
    if (mightContainLink(message) && recently_posted[message.author.id].filter(stored => messagesEqual(message, stored)).length == MESSAGES_TO_CACHE) {
        // delete the recent messages
        for (stored of recently_posted[message.author.id]) {
        ...
        + `${recently_posted[message.author.id].map(m => m.channel.toString()).join(', ')}, content:\n\`\`\`\n${message.content}\n\`\`\``).catch(console.error)
...
setInterval(function() {
    ...
    for (key in recently_posted) {
        // remove old enough items from the queue
        while (recently_posted[key][0]?.createdTimestamp < current_time - 1000 * 60 * PURGE_INTERVAL_MINUTES) {
            recently_posted[key].shift()
        }
        // delete message cache entry for user if empty
        if (recently_posted[key].length == 0) {
            delete recently_posted[key]
        }
    }
